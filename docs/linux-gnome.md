...
- Use the system's native notification, lock-screen, display, network, and power behavior. Use the approved Clipboard Indicator exception below for clipboard history.
...
- Approved exception for this environment: install [Touchpad Gesture Customization version 23](https://extensions.gnome.org/extension/7850/touchpad-gesture-customization/) from the GNOME Extensions catalog. It supports GNOME 45--48. Keep normal scrolling, set three-finger vertical swipe to Overview navigation with its default Overview direction enabled, and disable its other swipe and pinch actions. Log out and in after installation so GNOME Shell discovers it, then run `gnome-extensions enable touchpad-gesture-customization@coooolapps.com` and require `gnome-extensions info` to report `Enabled: Yes` and `State: ACTIVE`. Revalidate the gesture after every GNOME Shell update.
- Approved clipboard exception: install Debian's `gnome-shell-extension-manager` package, then use Extension Manager to install the current compatible release of [Clipboard Indicator](https://extensions.gnome.org/extension/779/clipboard-indicator/) (`clipboard-indicator@tudmotu.com`) for clipboard history. Enable only this and the approved Touchpad Gesture Customization extension. Install Debian's `wl-clipboard` package so Wayland terminal applications can read clipboard image data. In Pi under Ghostty, use `Ctrl+V` to paste an image; Ghostty's `Ctrl+Shift+V` remains text paste.
...
- When the approved Touchpad Gesture Customization extension is configured to preserve normal scrolling and map three fingers up to the Overview, verify that behavior manually after a fresh GNOME login. Confirm `gnome-extensions info touchpad-gesture-customization@coooolapps.com` reports `Enabled: Yes` and `State: ACTIVE`. If it fails, leave setup incomplete and record the GNOME Shell version and relevant `overview` or gesture errors.
- `gnome-extensions info clipboard-indicator@tudmotu.com` reports `Enabled: Yes` and `State: ACTIVE`. Confirm `gnome-extensions list --enabled` contains only Clipboard Indicator and Touchpad Gesture Customization.
- `command -v extension-manager wl-copy wl-paste` finds all three tools. In the GNOME Wayland session, copy text, confirm it appears in Clipboard Indicator, select it from history, and paste it into another application. Then copy an image and paste it into Pi with `Ctrl+V` in Ghostty.
